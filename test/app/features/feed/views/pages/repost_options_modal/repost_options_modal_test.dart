...
void main() {
  ...
  group('RepostOptionsModal', () {
    ...
    testWidgets('persists repost state through OptimisticService', (tester) async {
      final repostedState = PostRepostFactory.createReposted();

      await tester.pumpWithHarness(
        childBuilder: (_) => const RepostOptionsModal(eventReference: eventReference),
        overrides: [
          postRepostWatchProvider(eventReference.toString()).overrideWith(
            (ref) => Stream.value(repostedState),
          ),
        ],
        localizationsDelegates: I18n.localizationsDelegates,
        supportedLocales: I18n.supportedLocales,
    ...
    testWidgets('maintains state after backend sync', (tester) async {
      final streamController = StreamController<PostRepost?>();
      ...
      streamController.add(PostRepostFactory.createNotReposted(repostsCount: 3));
      await tester.pumpAndSettle();

      expect(find.byType(ModalActionButton), findsNWidgets(2));

      streamController.add(
        PostRepostFactory.createReposted(repostsCount: 4),
      );
      await tester.pumpAndSettle();
      ...
      streamController.add(
        PostRepostFactory.createReposted(
          repostsCount: 4,
          myRepostReference: const ImmutableEventReference(
            masterPubkey: 'backend-generated',
            eventId: 'actual-repost-id',
            kind: 6,
          ),
        ),
      );
