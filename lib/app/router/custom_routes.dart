...
mixin CupertinoRouteTransitionMixin<T> on PageRoute<T> {
  ...
  bool canTransitionTo(TransitionRoute<dynamic> nextRoute) {
    ...
    final nextRouteIsNotFullscreen = (nextRoute is! PageRoute<T>) || !nextRoute.fullscreenDialog;
...
class _CupertinoPageTransitionState extends State<CupertinoPageTransition> {
  ...
  void _setupAnimation() {
    ...
    _primaryPositionAnimation =
        (_primaryPositionCurve ?? widget.primaryRouteAnimation).drive(_kRightMiddleTween);
    _secondaryPositionAnimation =
        (_secondaryPositionCurve ?? widget.secondaryRouteAnimation).drive(_kMiddleLeftTween);
...
class _CupertinoEdgeShadowPainter extends BoxPainter {
  ...
  void paint(Canvas canvas, Offset offset, ImageConfiguration configuration) {
    ...
    final shadowWidth = 0.05 * configuration.size!.width;
    ...
    for (var dx = 0; dx < shadowWidth; dx += 1) {
      ...
      canvas.drawRect(Rect.fromLTWH(x - 1.0, offset.dy, 1, shadowHeight), paint);
    }
  }
}
