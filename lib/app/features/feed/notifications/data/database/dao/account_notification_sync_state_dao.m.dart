...
AccountNotificationSyncStateDao accountNotificationSyncStateDao(Ref ref) =>
    AccountNotificationSyncStateDao(ref.watch(accountNotificationsDatabaseProvider));

@DriftAccessor(tables: [AccountNotificationSyncStateTable])
class AccountNotificationSyncStateDao extends DatabaseAccessor<AccountNotificationsDatabase>
  ...
  Future<AccountNotificationSyncState?> getByRelayAndContentType({
    required String relayUrl,
    required ContentType contentType,
  }) {
    return (select(accountNotificationSyncStateTable)
          ..where((t) => t.relayUrl.equals(relayUrl) & t.contentType.equalsValue(contentType)))
        .getSingleOrNull();
  }
