...
class ArticleCategoriesMenu extends HookConsumerWidget {
  ...
  Widget build(BuildContext context, WidgetRef ref) {
    ...
    var visibleSubcategories = visibleCategoriesKeys
        .map(
          (key) => availableCategories[key] != null
              ? (key: key, category: availableCategories[key]!)
              : null,
        )
        .nonNulls
        .toList();
    ...
    if (visibleSubcategories.isEmpty) {
      visibleSubcategories = availableCategories.entries
          .map((entry) => (key: entry.key, category: entry.value))
          .toList();
    }
